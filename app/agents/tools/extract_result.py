...
extract_result_json = {
    ...
    "parameters": {
        ...
        "properties": {
            "Result": {
                "type": "string",
                "description": "Извлеченный результат из собеседования. Если результат не указан, не выдумывайте его, просто укажите, что результат не описан.",
            }
        },
        "required": ["Result"],
...
def extract_result(messages: List[Message]) -> str:
    ...
    response = ttt.generate_response_with_function(
        messages=messages, functions=[extract_result_json]
    )
